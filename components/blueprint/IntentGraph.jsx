  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '600px' }}>
      ...
      <div style={{
        ...
        <button onClick={() => addNode('screen')} style={toolbarBtnStyle('#1d4ed8', '#eff6ff')}>
          + Screen
        </button>
        <button onClick={() => addNode('feature')} style={toolbarBtnStyle('#15803d', '#f0fdf4')}>
          + Feature
        </button>
        <button onClick={() => addNode('database')} style={toolbarBtnStyle('#c2410c', '#fff7ed')}>
          + Table
        </button>
        <div style={{ flex: 1 }} />
        <span style={{ fontSize: '12px', color: '#888' }}>
          Drag nodes • Connect by dragging from dot to dot
        </span>
      </div>
