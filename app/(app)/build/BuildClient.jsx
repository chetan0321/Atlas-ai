...
// Receives projectId directly from the server-component page.js —
// no useSearchParams() needed, works perfectly on Vercel.
    ...
    try {
      ...
      if (data.riskReport) {
        ...
        await fetch('/api/project/save', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ action: 'risk', projectId, blueprintId: currentBlueprintId, riskReport: data.riskReport })
        })
      } else {
  ...
  if (resuming) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0d0d18' }}>
        <style>{`
          @keyframes res-pulse { 0%,100%{opacity:0.35} 50%{opacity:0.7} }
          @keyframes res-spin  { to { transform: rotate(360deg); } }
          .res-sk { animation: res-pulse 1.4s ease-in-out infinite; background: rgba(255,255,255,0.07); border-radius: 8px; }
        `}</style>
        <div style={{ height: '56px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '24px', flexShrink: 0 }}>
          {[1,2,3,4].map(i => <div key={i} className="res-sk" style={{ height: '28px', width: '100px', animationDelay: `${i * 0.1}s` }} />)}
        </div>
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ textAlign: 'center' }}>
            <div style={{ width: '40px', height: '40px', margin: '0 auto 16px', border: '3px solid rgba(139,92,246,0.2)', borderTop: '3px solid #7c3aed', borderRadius: '50%', animation: 'res-spin 0.8s linear infinite' }} />
            <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '14px' }}>Loading project…</p>
          </div>
        </div>
      </div>
    )
  ...
  return (
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0d0d18', position: 'relative' }}>
        ...
        {STEPS.map((s, i) => {
          ...
          return (
            <div key={s.key} style={{ display: 'flex', alignItems: 'center' }}>
              {i > 0 && <div style={{ width: '60px', height: '1px', background: done ? 'rgba(139,92,246,0.5)' : 'rgba(255,255,255,0.1)' }} />}
              <div
                className="build-step-pill"
                onClick={() => clickable && goToStep(s.key)}
                style={{
                  display: 'flex', alignItems: 'center', gap: '7px',
                  padding: '5px 14px 5px 6px', borderRadius: '20px',
              ...
              >
                ...
                <span style={{ fontSize: '13px', fontWeight: active ? '600' : '400', color: active ? '#fff' : done ? 'rgba(255,255,255,0.7)' : 'rgba(255,255,255,0.4)' }}>
                  {s.label}
                </span>
              </div>
            </div>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '28px 28px 220px', background: '#08080f' }}>
        ...
        {/* ── IDLE STAGE ── */}
        {stage === 'idle' && (
          <div className="build-section-enter" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '60vh', textAlign: 'center', padding: '20px 20px 0' }}>
            ...
            <h2 style={{ fontSize: 'clamp(28px, 4vw, 40px)', fontWeight: '700', color: '#fff', marginBottom: '12px', letterSpacing: '-0.5px', lineHeight: '1.15' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(195px, 1fr))', gap: '10px', maxWidth: '860px', width: '100%' }}>
              {[
                ...
                <div key={t.name} className="build-tmpl-card"
                  onClick={() => setInputValue(`Build me a ${t.name.toLowerCase()}: ${t.desc}`)}
                  style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '10px', padding: '14px 16px', cursor: 'pointer', textAlign: 'left' }}
                >
            ...
            <div style={{ marginBottom: '40px' }}>
              ...
              {researchError && (
                <div style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: '10px', padding: '14px 18px', marginBottom: '16px', textAlign: 'center' }}>
                  <p style={{ color: '#f87171', fontSize: '13px', fontWeight: '600', margin: 0 }}>Something went wrong researching your idea. Please try again.</p>
              ...
              {loading && !research ? (
                <div style={{ background: '#15151f', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '4px 12px 12px 12px', padding: '24px' }}>
                  {[100, 85, 90, 70, 80, 60].map((w, i) => (
                    <div key={i} className="build-skeleton" style={{ height: '13px', width: `${w}%`, background: 'rgba(255,255,255,0.08)', borderRadius: '6px', marginBottom: i < 5 ? '12px' : 0, animationDelay: `${i * 0.1}s` }} />
                  ))}
                </div>
              ) : (
                <div style={{ background: '#15151f', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '4px 12px 12px 12px', padding: '20px 24px', fontSize: '14px', lineHeight: '1.85', color: 'rgba(255,255,255,0.75)', whiteSpace: 'pre-wrap', animation: research ? 'build-fadein 0.2s ease both' : 'none' }}>
                  {research}
                </div>
              )}
            </div>
            {stage === 'research' && !loading && (
              <div className="build-section-enter" style={{ display: 'flex', justifyContent: 'center', marginTop: '32px', marginBottom: '20px' }}>
                <button onClick={handleGenerateBlueprint} disabled={blueprintLoading}
                  style={{ background: blueprintLoading ? 'rgba(139,92,246,0.3)' : '#7c3aed', color: '#fff', border: 'none', padding: '12px 32px', fontSize: '14px', fontWeight: '600', borderRadius: '10px', cursor: blueprintLoading ? 'not-allowed' : 'pointer', transition: 'all 0.15s', display: 'flex', alignItems: 'center', gap: '8px' }}
        ...
        {stage === 'blueprint' && blueprint && (
          <div className="build-slide-enter" style={{ maxWidth: '900px', margin: '24px auto 0' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px', flexWrap: 'wrap' }}>
              <button onClick={() => setStage('research')} style={{ background: 'transparent', color: 'rgba(255,255,255,0.4)', border: '1px solid rgba(255,255,255,0.1)', padding: '5px 13px', borderRadius: '7px', fontSize: '12px', fontWeight: '500', cursor: 'pointer', transition: 'all 0.15s', flexShrink: 0 }}
        ...
        {stage === 'risk' && (
          <div className="build-slide-enter" style={{ maxWidth: '900px', margin: '24px auto 0' }}>
            <div style={{ marginBottom: '20px' }}>
              <button onClick={() => setStage('blueprint')} style={{ background: 'transparent', color: 'rgba(255,255,255,0.4)', border: '1px solid rgba(255,255,255,0.1)', padding: '5px 13px', borderRadius: '7px', fontSize: '12px', fontWeight: '500', cursor: 'pointer', transition: 'all 0.15s' }}
                onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.06)'; e.currentTarget.style.color = '#fff' }}
                onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'rgba(255,255,255,0.4)' }}
              >← Blueprint</button>
            </div>
            {riskLoading ? (
              <div style={{ textAlign: 'center', padding: '80px 20px' }}>
                <div style={{ width: '52px', height: '52px', margin: '0 auto 20px', border: '3px solid rgba(139,92,246,0.2)', borderTop: '3px solid #7c3aed', borderRadius: '50%', animation: 'build-spin 0.9s linear infinite' }} />
        ...
        {/* ── SAVED STAGE ── */}
        {stage === 'saved' && (
          <div className="build-section-enter" style={{ maxWidth: '500px', margin: '40px auto', textAlign: 'center', background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.25)', borderRadius: '16px', padding: '48px 32px' }}>
            <div style={{ fontSize: '52px', marginBottom: '16px' }}>🎉</div>
            <h2 style={{ fontSize: '24px', fontWeight: '800', marginBottom: '8px', color: '#fff' }}>Saved to Dashboard!</h2>
            ...
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap' }}>
              <a href="/dashboard" style={{ background: '#fff', color: '#000', border: 'none', padding: '10px 24px', borderRadius: '8px', fontSize: '14px', fontWeight: '700', textDecoration: 'none' }}>View all projects →</a>
              <button onClick={reset} style={{ background: 'transparent', color: 'rgba(255,255,255,0.5)', border: '1px solid rgba(255,255,255,0.15)', padding: '10px 24px', borderRadius: '8px', fontSize: '14px', fontWeight: '500', cursor: 'pointer' }}>Build another app</button>
            </div>
          </div>
        )}
      </div>
      ...
      {(stage === 'idle' || stage === 'research') && (
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '16px 32px 24px', background: 'linear-gradient(to top, #0d0d18 65%, transparent)' }}>
          <div style={{ maxWidth: '840px', margin: '0 auto' }}>
            <textarea
              ...
              style={{ width: '100%', minHeight: '100px', maxHeight: '180px', padding: '16px 18px', fontSize: '14px', border: '1.5px solid rgba(139,92,246,0.45)', outline: 'none', resize: 'none', color: '#fff', background: 'rgba(255,255,255,0.03)', boxSizing: 'border-box', lineHeight: '1.6', borderRadius: '12px', transition: 'border-color 0.15s' }}
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '10px' }}>
              ...
              <button onClick={handleResearch} disabled={loading || !inputValue.trim()}
                style={{ background: loading || !inputValue.trim() ? 'rgba(139,92,246,0.3)' : '#7c3aed', color: '#fff', border: 'none', padding: '9px 22px', fontSize: '13px', fontWeight: '600', borderRadius: '8px', cursor: loading || !inputValue.trim() ? 'not-allowed' : 'pointer', transition: 'all 0.15s', display: 'flex', alignItems: 'center', gap: '6px' }}
              >
