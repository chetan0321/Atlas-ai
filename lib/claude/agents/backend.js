...
export async function backendAgent(blueprint, tier, templateSections = []) {
  if (tier === 1) return {}

  const templateContext = templateSections.length > 0
    ? `\n\nTEMPLATE API SECTIONS TO REFERENCE:\n${formatSectionsForPrompt(templateSections)}\n\nMatch these API route patterns and adapt them for this project.`
    : ''
