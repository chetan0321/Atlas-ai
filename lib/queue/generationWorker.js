...
// Keep the worker alive through transient Redis drops / network blips
process.on('uncaughtException', (err) => {
  if (err.code === 'ECONNRESET' || err.code === 'ETIMEDOUT' || err.code === 'EPIPE') {
    console.warn('[Worker] Recoverable network error (ignoring):', err.code, err.message)
  } else {
    console.error('[Worker] Uncaught exception:', err)
    // Only exit on non-recoverable errors
    process.exit(1)
  }
})
...
process.on('unhandledRejection', (reason) => {
  ...
  // Don't exit — let BullMQ retry logic handle job failures
})
...
import { matchTemplate }         from '../template-engine/matcher.js'
import { getSectionsForAgent, preloadAllTemplates } from '../template-engine/loader.js'
...
const worker = new Worker(
  ...
  async (job) => {
    ...
    // ── 1. Template matching (runs before agents) ─────────────────────────────
    let templateMatch = { strategy: 'generate', template: null, confidence: 0, reason: 'No match' }
    const description = blueprintJson.description || blueprintJson.projectName || ''

    try {
      templateMatch = await matchTemplate(description)
      console.log(`[Worker] Template match: ${templateMatch.strategy} — ${templateMatch.template?.id || 'none'} (${templateMatch.confidence}%)`)
    } catch (err) {
      console.warn('[Worker] Template matching failed, using scratch generation:', err.message)
    }

    // Store template match result for analytics + Preview UI
    await supabase
      .from('generation_runs')
      .update({
        template_id:         templateMatch.template?.id || null,
        template_strategy:   templateMatch.strategy,
        template_confidence: templateMatch.confidence,
        template_reason:     templateMatch.reason,
      })
      .eq('id', generationRunId)

    // ── 2. Load template sections for each agent (async, cached) ─────────────
    const useTemplate = templateMatch.strategy !== 'generate' && templateMatch.template !== null

    const [frontendSections, backendSections, schemaSections, securitySections, testSections] =
      await Promise.all([
        useTemplate ? getSectionsForAgent(templateMatch.template, 'frontend')  : Promise.resolve([]),
        useTemplate ? getSectionsForAgent(templateMatch.template, 'backend')   : Promise.resolve([]),
        useTemplate ? getSectionsForAgent(templateMatch.template, 'schema')    : Promise.resolve([]),
        useTemplate ? getSectionsForAgent(templateMatch.template, 'security')  : Promise.resolve([]),
        useTemplate ? getSectionsForAgent(templateMatch.template, 'test')      : Promise.resolve([]),
      ])

    // ── 3. Run 5 agents in parallel (with template sections) ─────────────────
    const [frontend, backend, schema, security, test] = await Promise.allSettled([
      frontendAgent(blueprintJson, tier, frontendSections),
      backendAgent(blueprintJson, tier, backendSections),
      schemaAgent(blueprintJson, tier, schemaSections),
      securityAgent(blueprintJson, tier, securitySections),
      testAgent(blueprintJson, tier, testSections),
    ])
